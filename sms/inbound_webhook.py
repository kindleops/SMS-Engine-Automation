"""Inbound SMS webhook that persists messages via the schema-aware datastore."""
...
from sms.datastore import promote_to_lead as _promote_to_lead
from sms.datastore import touch_lead
...
def log_conversation(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Create or update a conversation row based on inbound payload."""

    message_sid = payload.get("message_sid")
    fields: Dict[str, Any] = {
        CONV_FROM_FIELD: payload.get("from_number"),
        CONV_TO_FIELD: payload.get("to_number"),
    ...
    if payload.get("to_number"):
        fields[CONV_TO_FIELD] = payload["to_number"]
    if message_sid:
...
def update_lead_activity(
    ...
    body: str,
    direction: str,
    reply_increment: bool = False,  # kept for compatibility with legacy callers/tests
) -> None:
    if not lead_id:
        return
...
def handle_inbound(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Process a non-opt-out inbound message."""

    from_number = normalize_e164(payload.get("From"), field="From")
    body = sanitize_body(payload.get("Body"))
    to_raw = payload.get("To")
    to_number = normalize_e164(to_raw, field="To") if to_raw else None
...
def process_optout(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Handle STOP/DNC requests and suppress future outreach."""
    ...
    if record and record.get("id"):
        update_conversation(
            record["id"],
            {
                ...
                CONV_PROCESSED_BY_FIELD: "Opt-Out",
            },
        )

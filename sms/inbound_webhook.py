...
def _digits(v: str | None) -> str: return "".join(ch for ch in str(v or "") if ch.isdigit())
def _e164(v: str | None, field="Phone") -> str:
    d = _digits(v)
    if len(d) == 10: return "+1" + d
    if len(d) == 11 and d.startswith("1"): return "+" + d
    if v and v.startswith("+"): return "+" + d
    raise HTTPException(status_code=422, detail=f"Invalid {field}")

def _sanitize(body: Any) -> str:
    txt = str(body or "").strip()
    if not txt: raise HTTPException(status_code=422, detail="Empty Body")
    return txt
...
def _log_conv(data: dict, lead_id: str | None, prospect_id: str | None, status="DELIVERED", processed_by="Inbound Webhook"):
    ...
    if data.get("sid"): fields["TextGrid ID"] = data["sid"]
    if lead_id: fields["Lead"] = [lead_id]
    if prospect_id: fields["Prospect"] = [prospect_id]
    return create_conversation(data.get("sid"), fields)

def _update_lead(lead_id: str | None, body: str):
    if not lead_id: return
    try: touch_lead(lead_id, body=body, direction=ConversationDirection.INBOUND.value)
    except Exception: logger.warning("Lead touch failed", exc_info=True)
...
def _process_inbound(data: dict) -> dict:
    ...
    if to_n:
        try: increment_delivered(to_n)
        except Exception: logger.warning("Increment delivered failed", exc_info=True)
...
def _process_optout(data: dict) -> dict:
    ...
    if to_n:
        try: increment_opt_out(to_n)
        except Exception: logger.warning("Increment opt-out failed", exc_info=True)

    lead, prospect = ensure_prospect_or_lead(from_n)
    lead_id = (lead or {}).get("id")
    prospect_id = (prospect or {}).get("id")

    record = _log_conv({"from": from_n, "to": to_n, "body": body, "sid": sid}, lead_id, prospect_id, status="DNC", processed_by="Opt-Out Webhook")
    if lead_id:
        try:
    ...
    return {"status": "optout", "conversation_id": (record or {}).get("id"), "message_sid": sid, "linked": "lead" if lead_id else "prospect"}

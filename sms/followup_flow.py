...
def last10(v: Any) -> Optional[str]:
    d = _digits_only(v);  return d[-10:] if d else None
...
def _already_queued_today(drip: Any, phone: str) -> bool:
    ...
    try:
        ...
        if QUIET_TZ: today = today.astimezone(QUIET_TZ)
        prefix = today.strftime("%Y-%m-%d")
        p10 = last10(phone)
        for r in drip.all():
            f = r.get("fields", {})
            ph = f.get("phone") or f.get("Phone")
            if last10(ph) != p10: continue
...
def _escalate_nurture(stage: str) -> str:
    ...
    if stage not in NURTURE_CHAIN: return STAGE_NURTURE_30
...
def run_followups(limit: int = 1000) -> Dict[str, Any]:
    ...
    if QUIET_TZ: today_ct = today_ct.astimezone(QUIET_TZ)

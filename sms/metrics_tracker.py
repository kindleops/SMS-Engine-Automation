...
def _notify(msg):
    if ALERT_PHONE:
        ...
        except Exception as e: print(f"❌ SMS alert failed: {e}")
    if ALERT_EMAIL_WEBHOOK:
        try: requests.post(ALERT_EMAIL_WEBHOOK, json={"text": msg}, timeout=10)
        except Exception as e: print(f"❌ Webhook alert failed: {e}")
...
def update_metrics():
    ...
    for camp in campaigns.all():
        ...
        # Alerts
        if _should_alert(last_alert_at, optout_rate, THRESHOLD):
        ...
        if kpis:
            for metric, value in [
                ...
                try: kpis.create({"Campaign": camp_name,"Metric":metric,"Value":value,"Date":today})
                except: traceback.print_exc()
    ...
    if kpis:
        for metric, value in [
            ...
            try: kpis.create({"Campaign":"ALL","Metric":metric,"Value":value,"Date":today})
            except: traceback.print_exc()

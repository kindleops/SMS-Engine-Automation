...
# ─────────────────────────────────────────────────────────────
# pyairtable compat (v2: Api, v1: Table). Never hard-crash if missing.
# ─────────────────────────────────────────────────────────────
_PyApi = None
_PyTable = None
try:
...
try:
    ...
    def update_metrics(*_args, **_kwargs):
        return {"ok": True}
...
# Throughput
MESSAGES_PER_MIN = max(1, int(os.getenv("MESSAGES_PER_MIN", "20")))  # legacy/global cap
RATE_MAX_PER_NUMBER_PER_MIN = max(1, int(os.getenv("RATE_MAX_PER_NUMBER_PER_MIN", "20")))
...
def schedule_time(base_utc: datetime, idx: int, seconds_per_msg: int) -> str:
...
# Back-compat public aliases (other modules import these)
def get_time_field(fields: Dict[str, Any], *names: str) -> Optional[datetime]:
    return _get_time_field(fields, *names)


def parse_time_maybe_ct(value: Any) -> Optional[datetime]:
    return _parse_time_maybe_ct(value)
...
# ─────────────────────────────────────────────────────────────
# Template helpers (robust {First}, {Address}, etc.)
# ─────────────────────────────────────────────────────────────
def _augment_context(fields: Dict[str, Any]) -> Dict[str, str]:
    """
    Build a normalized, alias-rich map so {First} / {Address} and many variants resolve.
    """
    amap: Dict[str, str] = {}
    # 1) Copy all fields (normalized)
    for k, v in (fields or {}).items():
        amap[_norm(k)] = _stringify(v)

    # 2) First name
    first_candidates = [
        amap.get("first"),
        amap.get("firstname"),
        amap.get("ownerfirstname"),
        amap.get("sellerfirstname"),
        amap.get("contactfirstname"),
    ]
    if not any(first_candidates):
        full_name = (amap.get("name") or amap.get("fullname") or
                     amap.get("ownername") or amap.get("sellername") or "")
        if full_name:
            first_candidates.append(full_name.split()[0])
    first = next((x for x in first_candidates if x), "")
    if first:
        for alias in ("first", "firstname", "ownerfirst", "givenname"):
            amap[alias] = first

    # 3) Address (favor property/site over mailing)
    addr = (
        amap.get("address")
        or amap.get("propertyaddress")
        or amap.get("siteaddress")
        or amap.get("streetaddress")
        or amap.get("mailingaddress")
        or amap.get("propertystreet")
        or amap.get("street")
    )
    # compose from parts if needed
    if not addr:
        street = (amap.get("street") or amap.get("propertystreet") or amap.get("mailingstreet") or "")
        city = (amap.get("city") or amap.get("propertycity") or amap.get("mailingcity") or "")
        state = (amap.get("state") or amap.get("propertystate") or amap.get("mailingstate") or "")
        postal = (amap.get("zip") or amap.get("zipcode") or amap.get("postalcode") or "")
        bits = [b for b in [street, city, state] if b]
        addr = ", ".join(bits) + (f" {postal}" if postal else "")
        addr = addr.strip()
    if addr:
        for alias in ("address", "propertyaddress", "siteaddress"):
            amap.setdefault(alias, addr)
...
def _format_template(text: str, ctx: Dict[str, Any]) -> str:
    if not text:
        return text
    amap = _augment_context(ctx)

    def repl(m):
        raw = m.group(1) or m.group(2)
        val = amap.get(_norm(raw))
        return val if val not in (None, "") else m.group(0)
...
# ─────────────────────────────────────────────────────────────
# Numbers picker — market-aware; works with or without "Sent Today" columns.
# Also provides deterministic round-robin using phone hash when fields are missing.
# ─────────────────────────────────────────────────────────────
...
def _active_numbers(market: Optional[str]) -> List[Dict[str, Any]]:
...
def count_active_numbers(market: Optional[str]) -> int:
    return max(1, len(_active_numbers(market)))
...
def pick_from_number(market: Optional[str], salt: Optional[str] = None) -> Tuple[Optional[str], Optional[str]]:
    nums_tbl = get_numbers_table()
    if not nums_tbl:
        return (None, None)

    rows = _active_numbers(market)
    if not rows:
        return (None, None)

    # Detect whether the table actually has Remaining/Sent Today/Daily Reset fields
    fmap = _auto_field_map(nums_tbl)
    has_remaining = "remaining" in fmap
    has_sent_today = "senttoday" in fmap
    has_daily_reset = "dailyreset" in fmap
    has_last_used = "lastused" in fmap

    # If we have usable counters, pick the most remaining (then least recently used)
    if has_remaining or has_sent_today or has_daily_reset:
        elig: List[Tuple[Tuple[int, datetime], Dict]] = []
        for r in rows:
            f = r.get("fields", {})
            rem = f.get(fmap.get("remaining", "Remaining")) if has_remaining else None
            try:
                rem = int(rem) if rem is not None else None
            except Exception:
                rem = None
            if rem is None:
                sent_today = int(f.get(fmap.get("senttoday", "Sent Today")) or 0) if has_sent_today else 0
                daily_cap = int(f.get(fmap.get("dailyreset", "Daily Reset")) or DAILY_LIMIT_FALLBACK) if has_daily_reset else DAILY_LIMIT_FALLBACK
                rem = max(0, daily_cap - sent_today)
            if rem <= 0:
                continue
            last_used = _parse_dt(f.get(fmap.get("lastused", "Last Used"))) or datetime(1970, 1, 1, tzinfo=timezone.utc)
            elig.append(((-rem, last_used), r))

        if not elig:
            return (None, None)

        elig.sort(key=lambda x: x[0])  # most remaining; if tie, oldest last_used
        chosen = elig[0][1]
        ...
        # Update counters only if those fields exist
        update_payload: Dict[str, Any] = {}
        if has_sent_today:
            update_payload[fmap.get("senttoday", "Sent Today")] = int(cf.get(fmap.get("senttoday", "Sent Today")) or 0) + 1
        if has_last_used:
            update_payload[fmap.get("lastused", "Last Used")] = iso_now()
        if update_payload:
            _safe_update(nums_tbl, chosen["id"], update_payload)

        return did, chosen["id"]

    # Otherwise: deterministic round-robin using the phone/campaign salt
    idx = _stable_index(salt or iso_now(), len(rows))
    chosen = rows[idx]
    did = _to_e164(chosen.get("fields", {}))
    return (did, chosen["id"]) if did else (None, None)
...
def already_queued(drip_tbl, phone: str, campaign_id: str) -> bool:
    try:
        cutoff_dt = utcnow() - timedelta(hours=DEDUPE_HOURS)
        l10 = last10(phone)
        for r in drip_tbl.all():  # type: ignore[attr-defined]
            f = r.get("fields", {})
...
def run_campaigns(limit: int | str = 1, send_after_queue: Optional[bool] = None) -> Dict[str, Any]:
    ...
    picks market-aware numbers (with deterministic round-robin fallback),
    de-dupes, and optionally sends immediately.
    """
    ...
    for c in all_campaigns:
        ...
        # Require Go Live to be explicitly True
        go_live = f.get("Go Live")
        if go_live is not True:
            if DEBUG_CAMPAIGNS:
                print(f"[campaign] SKIP {name}: Go Live not checked")
            continue
        ...
        if start_dt and now_utc < start_dt and PREQUEUE_BEFORE_START:
            # Allow prequeue only if start is not inside quiet hours OR config allows it
            if not ALLOW_QUEUE_OUTSIDE_HOURS and _in_quiet_hours(start_dt):
                if DEBUG_CAMPAIGNS:
                    print(f"[campaign] WAIT {name}: start in quiet; prequeue disabled")
                continue
    ...
    for camp in eligible:
        ...
        # Decide base time (prequeue to Start; otherwise now or Start if already passed)
        ...
        # Effective throughput: 20 msgs / active number / minute (or env override).
        num_active = count_active_numbers(market)
        per_min = max(MESSAGES_PER_MIN, RATE_MAX_PER_NUMBER_PER_MIN * num_active)
        seconds_per_msg = max(1, math.ceil(60.0 / per_min))

        if prequeue:
            _safe_update(campaigns, cid, {"status": "Scheduled", "last_run_at": iso_now()})
        else:
        ...
        for idx, pr in enumerate(prospect_rows):
            ...
            from_number, number_rec_id = pick_from_number(market or pf.get("Market"), salt=last10(phone) or str(idx))
            if not from_number:
                continue
            ...
            scheduled_local = schedule_time(base_utc, idx, seconds_per_msg)
            ...
            payload = {
                ...
                "from_number": from_number,  # schema-safe; will map to "From Number" if present
                "status": "QUEUED",
                "next_send_date": scheduled_local,  # local (QUIET_TZ) naive
        ...
        if (not prequeue) and RUNNER_SEND_AFTER_QUEUE and queued > 0:
            try:
                batch_result = send_batch(campaign_id=cid, limit=per_min)
            except Exception:
                traceback.print_exc()
            if (batch_result.get("total_sent", 0) or 0) < queued:
                try:
                    retry_result = run_retry(limit=per_min, view="Failed Sends")
                except Exception:
                    retry_result = {}
        ...
        last_result = {
            ...
            "EffectiveMPM": per_min,
            "ActiveNumbers": num_active,
            "Prequeued": prequeue,
        }
        ...
        if DEBUG_CAMPAIGNS:
            print(
                f"[campaign] {name}: queued={queued}, sent_now={0 if prequeue else sent_delta}, "
                f"active_numbers={num_active}, mpm={per_min}, status→{new_status}"
            )

...
def build_lead_maps() -> Tuple[Dict[str,str], Dict[str,str], Dict[int,str]]:
    ...
    for r in rows:
        ...
        for k in LEAD_PHONE_FIELDS:
            ...
            if v:
                p10 = last10(v)
                if p10: phone_map.setdefault(p10, rid)
...
def build_template_map() -> Dict[str,str]:
    ...
    for r in tbl.all():
        ...
        if body: msg_to_rec.setdefault(body, rid)
...
def extract_conv_phone10(cf: dict) -> str:
    ...
    for k in CONV_DIR_FIELDS:
        if cf.get(k):
            direction = str(cf.get(k)).upper(); break
    order = (CONV_TO_FIELDS + CONV_FROM_FIELDS) if direction == "OUTBOUND" else \
            (CONV_FROM_FIELDS + CONV_TO_FIELDS) if direction == "INBOUND" else \
    ...
    for r in convs:
            ...
            if p10 and p10 in leads_phone:
                ...
                for k in ("Lead ID","lead_id","leadId","lead","Lead"):
                    ...
                    if isinstance(v, int) and v in leads_id_int:
                        lead_rec_to_set = leads_id_int[v]; break
                    s = str(v).strip()
                    if s and s in leads_id_str:
                        lead_rec_to_set = leads_id_str[s]; break
        ...
        if is_rec_id(curr_camp_text):
            ...
            for k in ("Campaign ID","campaign_id","Campaign","campaign"):
                ...
                key = str(v).strip()
                if key in camp_map:
                    camp_rec_to_set = camp_map[key]; break
            ...
            if p10 and p10 in pros_phone:
                ...
                for k in ("Lead ID","lead_id","leadId"):
                    ...
                    if isinstance(v, int) and v in pros_id_int:
                        pros_rec_to_set = pros_id_int[v]; break
                    s = str(v).strip()
                    if s and s in pros_id_str:
                        pros_rec_to_set = pros_id_str[s]; break
